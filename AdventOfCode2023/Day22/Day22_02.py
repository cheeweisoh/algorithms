...
def calcChainReaction(bricks: list[list[list[int]]]) -> int:
    space = calcFall(bricks)
    supporting, supported = findSupports(bricks, space)
    ans = 0

    for idx, _ in enumerate(bricks, start=1):
        disintegrated = set()
        queue = deque()
        queue.append(idx)
        ...
        while queue:
            brk = queue.popleft()
            # check if block has been disintegrated
            if brk in disintegrated:
                continue
            disintegrated.add(brk)

            for supported_brk in supporting[brk]:
                # check if supported brk has already disintegrated
                if supported_brk in disintegrated:
                    continue
                # check if supported_brk has other supports and other supports have not been disintegrated
                # if other supports have been disintegrated, this brick is also in queue to be disintegrated
                if (supported[supported_brk] and not supported[supported_brk] - disintegrated):
                    queue.append(supported_brk)
        ans += len(disintegrated) - 1
...
def main():
    ...
    print(f"Test Answer: {calcChainReaction(test_bricks)}")
    print(f"Puzzle Answer: {calcChainReaction(puzzle_bricks)}")
